...
trait SluggableTrait {
    ...
    protected function get_setting($settings, $key)
    {
    ...
    {
        $save_to = $this->get_setting($this->sluggable,'save_to');
        $on_update = $this->get_setting($this->sluggable,'on_update');
    ...
    {
        $separator  = $this->get_setting($this->sluggable,'separator');
        $method     = $this->get_setting($this->sluggable,'method');
        $max_length = $this->get_setting($this->sluggable,'max_length');
        if ( $method === null )
        {
    ...
    {
        $reserved = $this->get_setting($this->sluggable,'reserved');
        ...
        if ( is_array($reserved) )
        {
            ...
            {
                return $slug . $this->get_setting($this->sluggable,'separator') . '1';
            }
            return $slug;
        }
